...
RSpec.describe Event, type: :model do
  before do
    ...
    @e2 = Event.create({
      ...
      starts_at: 2.days.ago.utc,
      ends_at: 1.days.from_now.utc,  
  ...
  describe ".upcoming" do
    it "should show only events in future" do
      expect(Event.upcoming).to include @e1
    end
     
    it "should filter events in the past" do
      expect(Event.upcoming).not_to include @e2
    end     
  end

  describe ".search" do
    it "should search keyword in title" do
      expect(Event.search 'nam').to include @e1
    end
    ...
    it "should search keyword in description" do
      expect(Event.search 'Đan Trường').to include @e2
    end     
  end

  describe ".viewable" do
    it "should return published events of other users" do
      expect(Event.viewable @user1.id).to include @e3
    end

    it "should exclude non-published events of other users" do
      expect(Event.viewable @user2.id).not_to include @e2
    end

    it "should return all events of current user" do
      expect(Event.viewable @user1.id).to include @e1, @e2
    end
  end
end
